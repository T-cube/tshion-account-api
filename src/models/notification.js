...
export const extendedProps = [
  ...
  ['app.store.com.tlifang.activity.item', 'name,type,departments,company_id,is_public,creator,time_start', 'activity'],
...
export default class Notification {
  ...
  sendToMany(list, type, data) {
    let promises = list.sort().map((user, index) => {
      if(!user || user.equals(list[index-1]||{})) return null;
